...
namespace VMATCSIAutoPlanMT.VMAT_CSI
{
    public class GenerateTS_CSI : GenerateTSbase
    {
        ...
        protected override bool CalculateNumIsos()
        {
            ...
            //For these cases the maximum number of allowed isocenters is 3 (working to change to 4 isocenters). One isocenter is reserved for the brain and either one or two isocenters are used for the spine (depending on length).
            ...
            foreach (Tuple<string, List<string>> itr in planIdTargets)
            {
                ...
                if (string.Equals(longestTargetInPlan.Id, TargetsHelper.GetHighestRxTargetIdForPlan(prescriptions, prescriptions.First().Item1)))
                {
                    ...
                    numVMATIsos = CalculateNumVMATIsosForPTVCSI(spineTargetExtent, brainInfMargin, maxFieldExtent, minFieldOverlap)+1;
                    ProvideUIUpdate(100 * ++counter / calcItems, $"Final calculated number of VMAT isocenters: {numVMATIsos}");
                }
                else
                {
                    numVMATIsos = (int)Math.Ceiling(maxTargetLength / (maxFieldExtent - minFieldOverlap))+1;
