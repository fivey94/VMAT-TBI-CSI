...
namespace VMATCSIAutoPlanMT.VMAT_CSI
{
    public class PlaceBeams_CSI : PlaceBeamsBase
    {
                              ...
                              string mr, bool overlap, double overlapMargin, bool closePW)
        {
            ...
            collRot = coll;
            ebmpArc = new ExternalBeamMachineParameters(linac, "6X", 800, "ARC", "FFF");
        ...
        private (bool, List<Tuple<VVector, string, int>>) CalculateIsoPositionsForCSIInit(ExternalPlanSetup thePlan, int numIsos, double xUserOrigin)
        {
            ...
            for (int i = 0; i < numIsos; i++)
            {
                ...
                else
                {
                    ...
                    double distanceBetweenBrainandSpine = Math.Abs((brainZCenter-40) - (spineZMin+100));
                    double InteriorSeparation = Math.Round(distanceBetweenBrainandSpine / (numIsos - 1), 1);
                    v.z = (brainZCenter-40) - i * InteriorSeparation;

                    //Used to equally space the isocenters.
                    //v.z = (brainZCenter-40) -  i * isoSeparation;
            ...
            if (numIsos < 4)
            {
                for (int i = 0; i < numIsos; i++)
                {
                    ...
                    else
                    {
                        ...
                        //This if statement is new to change the distance of the final isocenter for a 3 iso plan.
                        if (i == 2) v.z = spineZMin + (numIsos - i - 1) * isoSeparation + 100.0;
                        /*
                         if (i == 1 && numIsos > 2)
                        {
                            //if this is iso 2 and the total number of isos is 3, apply this special logic to balance field
            ...
            for (int i = 0; i < numIsos; i++)
            {
                ...
                for (int j = 0; j < iso.Item2.ElementAt(i).Item3; j++)
                {
                    ...
                    */
                    ...
                    if (count % 2 == 0)
                    {
                        b = iso.Item1.AddMLCArcBeam(ebmpArc, leaves, jaws, coll, CCW[0], CCW[1], GantryDirection.CounterClockwise, 0, iso.Item2.ElementAt(i).Item1);
                    ...
                    {
                        b = iso.Item1.AddMLCArcBeam(ebmpArc, leaves, jaws, coll, CW[0], CW[1], GantryDirection.Clockwise, 0, iso.Item2.ElementAt(i).Item1);
